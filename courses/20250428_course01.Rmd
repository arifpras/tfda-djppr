...
```{r pitch}
# === Load packages ===
pacman::p_load(ggplot2, ggrepel, ggsoccer, dplyr, tidyr, forcats, egg, ggpubr)
...
ggpairs(
  ...
  lower = list(continuous = wrap("points", alpha = 0.6)),  # Scatterplots
  upper = list(continuous = wrap("cor", size = 4)),        # Correlation values
  diag  = list(continuous = wrap("barDiag", bins = 10))    # Histograms
)
...
```{r save_png}
ggsave(
  filename = "20250522_course01_plot01.png",
  plot = plot_point,
  width = 3840,
  height = 2160,
  units = "px",
...
```{r save_pdf}
# ggsave(
#   filename = "20250522_rcourse_plot01.pdf",
#   plot = plot_point,
#   device = cairo_pdf,
#   width = 297,
#   height = 210,
